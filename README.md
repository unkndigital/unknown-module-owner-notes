...
Local-only example module. Saves one note in its own private data directory; no TV settings or network access.
...
Output: dist/owner-notes-1.1.0.zip and dist/SHA256SUMS.txt. PC architecture is independent of the TV JavaScript runtime. The initial reviewed candidate ZIP and its checksum are also at the repository root. Checksums verify bytes, not publisher identity or safety.

## Install

In Core, choose Install Modules from GitHub and enter https://github.com/unkndigital/unknown-module-owner-notes. Select the module ZIP from Releases and review it before installation. Activation is a separate choice. You can also transfer the module ZIP to /media/internal/.unknown-core/modules-inbox through your owner-controlled connection. Use the module ZIP, not GitHub's automatic source-code ZIP.
...
Removes this example's activation marker and stops its actions. Your saved note stays in the module's data directory; turning the module off does not erase it.
